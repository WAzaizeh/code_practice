...
def all_vowel_pairs(words)
    ...
    words.combination(2) do |w1, w2|
        res << [w1,w2].join(' ') if VOWELS_SET == vowels_only(w1 + w2).chars.to_set
    end
    res
end
...
class String
    ...
    def substrings(length = nil)
        ...
        length.nil? ? res : res.select{|sub| sub.length == length}
    end
